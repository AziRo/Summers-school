...
int main()
{
    ...
    ep_event[0].events = EPOLLIN;
    ep_event[0].data.fd = fd_udp;
    epoll_ctl(epid, EPOLL_CTL_ADD, fd_udp, &ep_event[0]);

    ep_event[1].events = EPOLLIN;
    ep_event[1].data.fd = fd_tcp;
    epoll_ctl(epid, EPOLL_CTL_ADD, fd_tcp, &ep_event[1]);
    ...
    while (1) {
        ...
        /* Ожидание события */
        int ev_count = epoll_wait(epid, events, 2, -1);
        if (ev_count < 0) {
